...
export default function Freelancer() {
  ...
  const validateStepTwo = () => {
    ...
    if (!minRate && formData.paymentType === "hourly") {
      updatedErrors.minRate = "Enter minimum rate.";
      valid = false;
    }
    if (!maxRate) {
      updatedErrors.maxRate = formData.paymentType === "hourly" ? "Enter maximum rate." : "Enter project budget.";
      valid = false;
    }
    if (formData.paymentType === "hourly" && minRate && maxRate && Number(minRate) > Number(maxRate)) {
  ...
  const handlePaymentTypeChange = (type: "hourly" | "fixed") => {
    ...
    setFormData((prev) => ({
      ...prev,
      paymentType: type,
      // Reset rates when switching payment types
      minRate: "",
      maxRate: "",
    }));
  };
